...
export default function PinjamanPage() {
  ...
  async function handleSimpan() {
    ...
    if (!form.anggota_id || !form.unit_usaha_id || !form.total_amount || !form.akun_piutang_id || !form.akun_kas_id) {
      setError('Lengkapi semua field yang diperlukan'); return
    }
    const amount = parseFloat(form.total_amount)
    if (isNaN(amount) || amount <= 0) { setError('Jumlah pinjaman harus lebih dari 0'); return }
    ...
    try {
      ...
      logError('pinjamanPage.handleSimpan', 'Failed to save transaction', err)
    } finally {
      setSaving(false)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display text-gray-900">Transaksi Pinjaman</h1>
        <p className="text-gray-500 text-sm">Catat pinjaman anggota koperasi</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-semibold text-gray-900 mb-4">Form Pinjaman</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={form.jangka_waktu} onChange={e => setForm({ ...form, jangka_waktu: e.target.value })}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-merah/20 focus:border-merah">
                  {['3','6','12','24','36'].map(v => <option key={v} value={v}>{v} bulan</option>)}
                </select>
              </div>
              <div>
            ...
            {form.total_amount && parseFloat(form.total_amount) > 0 && (
              <div className="bg-yellow-50 border border-yellow-100 rounded-lg p-3 text-xs">
                <p className="font-medium text-yellow-800 mb-1">💡 Estimasi Angsuran Pokok:</p>
                <p className="text-yellow-700">Per bulan: <span className="font-mono font-semibold">
                  Rp {(parseFloat(form.total_amount) / parseInt(form.jangka_waktu)).toLocaleString('id-ID', { maximumFractionDigits: 0 })}
                </span></p>
              </div>
            )}
            <div className="p-3 bg-gray-50 rounded-lg space-y-3">
              ...
              <div>
                <label className="block text-xs text-gray-600 mb-1">Akun Piutang Anggota (Debit)</label>
            ...
            {form.total_amount && parseFloat(form.total_amount) > 0 && (
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs">
                ...
                <div className="font-mono space-y-1 text-blue-700">
                  <div className="flex justify-between"><span>DR Piutang Anggota</span><span>Rp {parseFloat(form.total_amount).toLocaleString('id-ID')}</span></div>
            ...
            <button onClick={handleSimpan} disabled={saving}
              ...
              {saving ? 'Memproses...' : 'Catat Pinjaman & Buat Jurnal'}
            </button>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h3 className="font-semibold text-gray-900">Riwayat Pinjaman</h3>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {transactions.length === 0 ? (
                  <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400 text-xs">Belum ada pinjaman</td></tr>
                ) : transactions.map(tx => (
