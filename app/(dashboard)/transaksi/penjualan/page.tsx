...
export default function PenjualanPage() {
  ...
  const [form, setForm] = useState({
    ...
    akun_kas_id: '', akun_pendapatan_id: '',
  })
  ...
  async function loadData() {
    ...
    const [unit, accounts, tx] = await Promise.all([
      ...
      supabase.from('transactions').select('*, unit_usaha(nama_unit)')
        .eq('koperasi_id', koperasiId).eq('jenis_transaksi', 'penjualan')
  ...
  async function handleSimpan() {
    ...
    const journalEntries = [
      ...
      { account_id: form.akun_pendapatan_id, debit: 0,      credit: amount },
    ]
    ...
    try {
      await saveTransaction({
        ...
        jenis_transaksi: 'penjualan', tanggal: form.tanggal,
        keterangan: form.keterangan || 'Penjualan tunai',
