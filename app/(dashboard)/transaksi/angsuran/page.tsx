...
export default function AngsuranPage() {
  ...
  async function loadData() {
    ...
    const [anggota, unit, accounts, tx] = await Promise.all([
      ...
      supabase.from('transactions').select('*, anggota(nama), unit_usaha(nama_unit)')
        .eq('koperasi_id', koperasiId).eq('jenis_transaksi', 'angsuran')
  ...
  async function handleSimpan() {
    ...
    const journalEntries = [
      ...
      { account_id: form.akun_piutang_id,  debit: 0,      credit: amount },
    ]
    ...
    try {
      await saveTransaction({
        ...
        anggota_id: form.anggota_id, jenis_transaksi: 'angsuran',
        tanggal: form.tanggal,
        keterangan: form.keterangan || 'Pembayaran angsuran pinjaman',
      ...
      logError('angsuranPage.handleSimpan', 'Failed to save transaction', err)
    } finally {
      setSaving(false)
