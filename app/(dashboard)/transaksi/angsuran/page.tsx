...
export default function AngsuranPage() {
  ...
  async function loadData() {
    ...
    const [anggota, unit, accounts, tx] = await Promise.all([
      ...
      supabase.from('transactions').select('*, anggota(nama), unit_usaha(nama_unit)')
        .eq('koperasi_id', userData.koperasi_id).eq('jenis_transaksi', 'angsuran')
  ...
  async function handleSimpan() {
    if (!form.anggota_id || !form.unit_usaha_id || !form.total_amount || !form.akun_kas_id || !form.akun_piutang_id) {
      setError('Lengkapi semua field yang diperlukan'); return
    }
    const amount = parseFloat(form.total_amount)
    if (isNaN(amount) || amount <= 0) { setError('Jumlah angsuran harus lebih dari 0'); return }

    setSaving(true); setError('')
    const supabase = createClient()
    const keterangan = form.keterangan || 'Pembayaran angsuran pinjaman'

    // Angsuran: DR Kas / CR Piutang Anggota
    const { data: tx, error: txErr } = await supabase.from('transactions').insert({
      koperasi_id: koperasiId, unit_usaha_id: form.unit_usaha_id, anggota_id: form.anggota_id,
      jenis_transaksi: 'angsuran', tanggal: form.tanggal, keterangan, total_amount: amount, created_by: userId,
    ...
    await supabase.from('journal_items').insert([
      ...
      { journal_id: journal.id, account_id: form.akun_piutang_id, debit: 0, credit: amount },
    ])

    setSuccess(`Angsuran berhasil dicatat! Rp ${amount.toLocaleString('id-ID')}`)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display text-gray-900">Transaksi Angsuran</h1>
        <p className="text-gray-500 text-sm">Catat pembayaran angsuran pinjaman anggota</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-semibold text-gray-900 mb-4">Form Angsuran</h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Jumlah Angsuran (Rp) *</label>
            ...
            <div className="p-3 bg-gray-50 rounded-lg space-y-3">
              ...
              <div>
                <label className="block text-xs text-gray-600 mb-1">Akun Piutang Anggota (Kredit)</label>
                <select value={form.akun_piutang_id} onChange={e => setForm({ ...form, akun_piutang_id: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-merah/20">
                  <option value="">Pilih akun piutang...</option>
                  {(piutangAccounts.length > 0 ? piutangAccounts : asetAccounts).map(a =>
                    <option key={a.id} value={a.id}>{a.kode_akun} - {a.nama_akun}</option>)}
                </select>
              </div>
            </div>
            ...
            {form.total_amount && parseFloat(form.total_amount) > 0 && (
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs">
                ...
                <div className="font-mono space-y-1 text-blue-700">
                  ...
                  <div className="flex justify-between pl-4"><span>CR Piutang Anggota</span><span>Rp {parseFloat(form.total_amount).toLocaleString('id-ID')}</span></div>
                </div>
              </div>
            )}
            ...
            <button onClick={handleSimpan} disabled={saving}
              ...
              {saving ? 'Memproses...' : 'Catat Angsuran & Buat Jurnal'}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h3 className="font-semibold text-gray-900">Riwayat Angsuran</h3>
          </div>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {transactions.length === 0 ? (
                <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400 text-xs">Belum ada angsuran</td></tr>
              ) : transactions.map(tx => (
