...
export default function AnggotaPage() {
  ...
  async function loadAnggota() {
    ...
    const { data: userData } = await supabase.from('users').select('koperasi_id').eq('id', user.user.id).single()
  ...
  async function handleSave() {
    ...
    if (editData) {
      ...
      await supabase.from('anggota').insert({ ...form, koperasi_id: userData?.koperasi_id })
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { setEditData(null); setForm({ nama: '', nik: '', alamat: '', no_hp: '', tanggal_masuk: new Date().toISOString().split('T')[0], status: 'aktif' }); setShowForm(true) }}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-16 text-gray-400">Memuat data...</div>
        ) : (
              ...
              {filtered.length === 0 ? (
                ...
                <tr key={a.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3 text-gray-500">
                    {a.tanggal_masuk ? new Date(a.tanggal_masuk).toLocaleDateString('id-ID') : '-'}
                  </td>
                  <td className="px-6 py-3">
                    <span className={a.status === 'aktif' ? 'badge-aktif' : 'badge-nonaktif'}>
                      {a.status}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={!form.nama || saving} className="flex-1 bg-merah text-white rounded-lg text-sm font-medium py-2 disabled:opacity-60 hover:bg-merah-dark">
